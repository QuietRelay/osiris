...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if ((activeLayers as any).alpr && flyToLocation && !layerFetchedRef.current.has('alpr')) {
      fetchEndpoint(`/api/alpr?lat=${flyToLocation.lat}&lng=${flyToLocation.lng}&radius=80&_t=${Date.now()}`);
      layerFetchedRef.current.add('alpr');
    }
    // Maritime
